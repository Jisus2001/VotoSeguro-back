...
router.get('/ObtenerPersona/:Identificacion', async(req,res)=>{
    console.log(Personas)
const personas = await Personas.findOne({ Identificacion:req.params.Identificacion });
res.status(200).json(personas)
})
...
Personas.updateOne({Identificacion:persona.Identificacion},{ $set: persona}, { runValidators: true })
...
const resultado = await Personas.deleteOne({ Identificacion:req.params.Identificacion });
...
const persona = await Personas.findOne({ Identificacion:req.body.Identificacion });
if(!persona){
    res.json({error: 'Datos invalidos' })
    return
}
if(persona.Contrasenna!==req.body.Contrasenna){
    res.json({error: 'Datos invalidos' })
    return
}
